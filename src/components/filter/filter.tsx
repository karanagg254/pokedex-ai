...
import { ActionTypes, Pokemon, PokemonDetail } from "../../models";
...
const AppFilter = ({ ...props }) => {
  ...
  const filterPokemonData = (data: PokemonDetail[]) => {
    dispatch({
  ...
  const setPokemonTypes = (data: Pokemon[]) => {
    if (data.length) {
      let newData: { label: string; value: string; url: string }[] = [];
  ...
  const setPokemonGendersList = (genderList: Pokemon[]) => {
    let newGenderList: { label: string; value: string; url: string }[] = [];
    newGenderList = genderList.map((item) => ({
      label: getCamleCaseString(item.name),
      value: item.url,
      url: item.url,
    }));
